...
DATASET_ROOT="${DATASET_ROOT:-${INTERIORAGENT_ROOT:-$HOME/InteriorAgent}}"
...
SKIP_COMPLETED_SCENES="${SKIP_COMPLETED_SCENES:-0}"
MIN_AVAILABLE_RAM_MB="${MIN_AVAILABLE_RAM_MB:-0}"
MIN_FREE_GPU_MB="${MIN_FREE_GPU_MB:-0}"
RESOURCE_POLL_SECONDS="${RESOURCE_POLL_SECONDS:-30}"
ROBOT_RADIUS_M="${ROBOT_RADIUS_M:-0.05}"
RUNTIME_PLANNING_CLEARANCE_M="${RUNTIME_PLANNING_CLEARANCE_M:-0.05}"
ASTAR_CLEARANCE_HARD_MIN_M="${ASTAR_CLEARANCE_HARD_MIN_M:-0.06}"
ASTAR_CLEARANCE_DESIRED_M="${ASTAR_CLEARANCE_DESIRED_M:-0.18}"
ASTAR_GOAL_MIN_CLEARANCE_M="${ASTAR_GOAL_MIN_CLEARANCE_M:-0.12}"
LOOKAHEAD_MIN_CLEARANCE_M="${LOOKAHEAD_MIN_CLEARANCE_M:-0.10}"
GUARD_MIN_CLEARANCE_M="${GUARD_MIN_CLEARANCE_M:-0.10}"
...
wait_for_resources() {
  ...
  while true; do
    ...
    gpu_free_mb=0
    if [[ "$MIN_FREE_GPU_MB" -gt 0 ]]; then
      if ! gpu_line="$(nvidia-smi --id=0 --query-gpu=memory.total,memory.used --format=csv,noheader,nounits | head -n 1)"; then
        echo "[voxroom-all] cannot query GPU 0 resources" >&2
        return 1
      fi
    ...
    if [[ "$available_ram_mb" -ge "$MIN_AVAILABLE_RAM_MB" && "$gpu_free_mb" -ge "$MIN_FREE_GPU_MB" ]]; then
      echo "[voxroom-all] resources ready for $scene_id ram_available_mb=$available_ram_mb gpu_free_mb=$gpu_free_mb"
      return 0
    fi
    echo "[voxroom-all] waiting for resources before $scene_id ram_available_mb=$available_ram_mb/$MIN_AVAILABLE_RAM_MB gpu_free_mb=$gpu_free_mb/$MIN_FREE_GPU_MB"
...
run_scene() {
  ...
  if ! wait_for_resources "$scene_id"; then
    echo "[voxroom-all] resource check failed for $scene_id" >&2
    return 1
  fi
  ...
  echo "[voxroom-all] ($run_index) running $scene_id threads=$RUN_NUMBA_NUM_THREADS"
  {
    ...
    echo "runtime_planning_clearance_m=$RUNTIME_PLANNING_CLEARANCE_M"
    echo "astar_clearance_hard_min_m=$ASTAR_CLEARANCE_HARD_MIN_M"
    echo "astar_clearance_desired_m=$ASTAR_CLEARANCE_DESIRED_M"
    echo "astar_goal_min_clearance_m=$ASTAR_GOAL_MIN_CLEARANCE_M"
    echo "lookahead_min_clearance_m=$LOOKAHEAD_MIN_CLEARANCE_M"
    echo "guard_min_clearance_m=$GUARD_MIN_CLEARANCE_M"
...
while IFS= read -r episode_file; do
  ...
  scene_id="$(basename "$episode_file" .jsonl)"
  if [[ "$SKIP_COMPLETED_SCENES" == "1" && -s "$RUN_ROOT/$scene_id/results.jsonl" ]]; then
    echo "[voxroom-all] skipping completed scene: $scene_id"
    continue
  fi
