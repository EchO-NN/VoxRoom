...
RUN_TAG="${RUN_TAG:-gibson_coverage_${SCENE_ID}_$(date +%Y%m%d_%H%M%S)}"
...
export ROOMSEG_COVERAGE_EVAL=1
export ROOMSEG_COVERAGE_MILESTONES="${ROOMSEG_COVERAGE_MILESTONES:-20,40,60,80,100}"
export TASK_CONFIG="tasks/pointnav_gibson_visual.yaml"
export SPLIT="val"
export REQUIRE_TOPOLOGY_TRANSITION=0
...
export WINDOW_CHECKPOINT_EVERY_STEPS="${WINDOW_CHECKPOINT_EVERY_STEPS:-100}"
