...
# Isaac Sim can retain large glibc arenas while loading GRScenes.  NVIDIA's
# documented allocator workaround keeps two standalone workers inside the
# host's RAM budget without changing rendering or sensor inputs.
export MALLOC_ARENA_MAX="${VOXROOM_MALLOC_ARENA_MAX:-1}"
export GLIBC_TUNABLES="${VOXROOM_GLIBC_TUNABLES:-glibc.malloc.arena_max=1:glibc.malloc.mmap_max=0:glibc.malloc.mmap_threshold=2147483647}"
