...
- 5 cm voxel/grid resolution.
- DDA endpoint marking at exactly the hit cell, with no 3x3 endpoint splat.
- Navigation projection from the no-clearance voxel navigation map.
- Door seeds constrained to no-clearance navigation free cells.
- Door memory freezing after a door has become a stable partition.
- Door seed height from `0.95 * estimated_ceiling_height`.
- Wall gap completion before door completion.
- No wall-line extension stage.
...
configs/
  voxroom_online.yaml
    Main Isaac, voxel mapping, navigation, and room-segmentation config.

voxroom_online/
  isaac_runtime/
    dataset/          InteriorAgent scene and episode utilities.
    env/              Isaac Sim launch and observation wrappers.
    mapping/          Voxel map, navigation projection, door seeds, room masks.
    navigation/       A* and random-frontier exploration.
    evaluation/       Online room-segmentation metrics and annotation tools.
    baselines/        Baseline adapters used by comparison scripts.
    visualization/    Debug visualization and HTML/PNG helpers.

scripts/
  run_one_scene_random_frontier.sh
    Run one Isaac Sim episode with random-frontier exploration.
  run_all_scenes_random_frontier.sh
    Run all generated InteriorAgent scene episodes.
  run_final_door_completion_scheme_20260610.sh
    Replay/evaluate the final door-completion snapshot scheme.
  render_final_door_completion_snapshots.py
    Render raw door-seed and checked door-completion snapshot views.
  comparison/
    Scripts for baseline, ablation, and gallery generation.

tests/
  Unit and smoke tests for the room-segmentation contract.

data/
  Placeholder for generated InteriorAgent preprocessing and episode JSONL files.

result/
  Placeholder for runtime outputs.
```
