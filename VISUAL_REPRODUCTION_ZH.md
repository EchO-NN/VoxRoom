...
记录确切 client window ID。严格模式实时画布固定为 `3200×1800`，同时锁定
窗口最小/最大尺寸并关闭手动缩放；启动器在整个回合持续核对窗口宽高，任一时刻
改变尺寸都会让本次验证失败。应用在第 5、10、30 个控制步完成真实渲染并保存
对应帧后，分别发出 first、later、mid 的 ready/ACK 请求，启动器只在握手身份
与进度流一致时采集物理 `seat0` 窗口；长回合每 100 控制步继续保存一次物理
窗口像素。算法完成后保持窗口，等待启动器完成终态窗口与桌面截图并回传确认，
再允许进程退出。保存的步频证据帧固定为 `1600×900`，最终图固定为
`1920×1080`；RGB、地图、拓扑和状态四个面板必须非空且从首帧到终帧变化。
