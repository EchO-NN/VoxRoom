...
class VoxRoomPopupVisualizer:
    def __init__(
        ...
        panel_size: Tuple[int, int] = (2200, 1400),
    ...
    def _save_panel_artifacts(self, step: int, panel: np.ndarray) -> None:
        step = int(step)
        if not self.save_dir or step < self._next_save_step:
            return
        try:
            Image.fromarray(panel).save(self.save_dir / ("voxroom_step_%06d.jpg" % step), format="JPEG", quality=85)
            if self.save_overlay_layer_metadata:
                meta_path = self.save_dir / ("voxroom_step_%06d.layers.json" % step)
                meta_path.write_text(
                    json.dumps(self.overlay_layer_metadata(step), ensure_ascii=False, indent=2) + "\n",
                    encoding="utf-8",
                )
            self._next_save_step = step + self.save_every_steps
        except Exception as exc:
    ...
    def _render_readme_panel(
        ...
        *,
        ...
        title_font = self._truetype_font(34, bold=True)
        panel_font = self._truetype_font(24, bold=True)
        ...
        content_bottom = panel_h - margin
        content_h = max(1, content_bottom - content_top)
        baseline_available = self._has_live_baseline_preview()
        if panel_w >= 1440 and panel_h >= 900:
            obs_h = max(420, min(int(content_h * 0.35), 520))
            map_h = max(1, content_h - obs_h - gutter)
            half_w = max(1, (panel_w - margin * 2 - gutter) // 2)
            if baseline_available:
                map_w = max(1, (panel_w - margin * 2 - gutter * 2) // 3)
                map_top = content_top + obs_h + gutter
                boxes = [
                    (margin, content_top, margin + half_w, content_top + obs_h),
                    (margin + half_w + gutter, content_top, panel_w - margin, content_top + obs_h),
                    (margin, map_top, margin + map_w, content_bottom),
                    (margin + map_w + gutter, map_top, margin + map_w * 2 + gutter, content_bottom),
                    (margin + map_w * 2 + gutter * 2, map_top, panel_w - margin, content_bottom),
                ]
            else:
                boxes = [
                    (margin, content_top, margin + half_w, content_top + obs_h),
                    (margin + half_w + gutter, content_top, margin + half_w * 2 + gutter, content_top + obs_h),
                    (margin, content_top + obs_h + gutter, margin + half_w, content_top + obs_h + gutter + map_h),
                    (margin + half_w + gutter, content_top + obs_h + gutter, margin + half_w * 2 + gutter, content_top + obs_h + gutter + map_h),
                ]
        else:
            cell_w = max(1, (panel_w - margin * 2 - gutter) // 2)
            if baseline_available:
                cell_h = max(1, (content_h - gutter * 2) // 3)
                boxes = [
                    (margin, content_top, margin + cell_w, content_top + cell_h),
                    (margin + cell_w + gutter, content_top, panel_w - margin, content_top + cell_h),
                    (margin, content_top + cell_h + gutter, margin + cell_w, content_top + cell_h * 2 + gutter),
                    (margin + cell_w + gutter, content_top + cell_h + gutter, panel_w - margin, content_top + cell_h * 2 + gutter),
                    (margin, content_top + cell_h * 2 + gutter * 2, panel_w - margin, content_bottom),
                ]
            else:
        ...
        baseline_preview = self._live_baseline_preview(crop_bounds)
        panel_labels = (
            ("RGB", "Depth", "Navigation Map + VoxRoom", "TVARS Original Mask", "Vertical Free Map")
            if baseline_preview is not None
            else ("RGB", "Depth", "Navigation Map + VoxRoom", "Vertical Free Map")
        )
        content_boxes = [
            self._readme_panel_box(draw, box, label, panel_font, border)
            for box, label in zip(boxes, panel_labels)
        ]
        ...
        if baseline_preview is None:
            self._paste_fit(
                panel,
                self._readme_vertical_free_image(map_shape, crop_bounds),
                content_boxes[3],
            )
        else:
            self._paste_fit(panel, baseline_preview, content_boxes[3])
            self._paste_fit(
                panel,
                self._readme_vertical_free_image(map_shape, crop_bounds),
                content_boxes[4],
            )
        model_rejected_seed_count = int(
            np.count_nonzero(self._room_debug_array("voxel_door_seed_model_reject_mask", map_shape, bool))
        )
        self._last_overlay_layers = [
            ...
            self._overlay_record("readme_vertical_free_panel", True, (122, 33, 158), 1, "vertical free map with purple kept seeds and dark-crimson model rejects"),
            self._overlay_record("readme_model_rejected_door_seed", bool(model_rejected_seed_count), MODEL_REJECTED_DOOR_SEED_COLOR, model_rejected_seed_count, "learned classifier rejected raw door seeds"),
        ]
        if baseline_preview is not None:
            self._last_overlay_layers.append(
                self._overlay_record(
                    "readme_active_room_original_panel",
                    True,
                    (216, 27, 96),
                    1,
                    "strict original TVARS colored room mask and accepted door lines",
                )
            )
    ...
    def _has_live_baseline_preview(self) -> bool:
        ...
        if not isinstance(debug, Mapping):
            return False
        payload = debug.get("live_baseline_visualization")
        if isinstance(payload, Mapping) and payload.get("preview_rgb") is not None:
            return True
        paths = debug.get("roomseg_snapshot_paths")
        if not isinstance(paths, Mapping):
            return False
        return bool(paths.get("tvars_original_isaac_png"))

    def _live_baseline_preview(
        self,
        ...
        payload = debug.get("live_baseline_visualization")
        if isinstance(payload, Mapping) and payload.get("preview_rgb") is not None:
            preview = np.asarray(payload["preview_rgb"], dtype=np.uint8)
            if preview.ndim != 3 or preview.shape[2] != 3:
                raise ValueError(
                    "TVARS live preview must be an HxWx3 uint8 image"
                )
            if crop_bounds is not None:
                r0, r1, c0, c1 = (int(value) for value in crop_bounds)
                if not (
                    0 <= r0 < r1 <= preview.shape[0]
                    and 0 <= c0 < c1 <= preview.shape[1]
                ):
                    raise ValueError(
                        "TVARS live preview does not match the runtime map crop"
                    )
                preview = preview[r0:r1, c0:c1]
            return Image.fromarray(preview, mode="RGB")
        paths = debug.get("roomseg_snapshot_paths")
        if not isinstance(paths, Mapping):
            return None
        raw_path = paths.get("tvars_original_isaac_png")
        if not raw_path:
            return None
        path = Path(str(raw_path)).expanduser()
        if not path.is_file():
            raise FileNotFoundError(
                "strict original Active Room colored room preview is missing: %s"
                % path
            )
    ...
    def _paste_fit(canvas: Image.Image, image: Image.Image, box: Tuple[int, int, int, int], *, resample=Image.Resampling.NEAREST) -> None:
        ...
        render_w = max(1, int(round(image.width * scale)))
        render_h = max(1, int(round(image.height * scale)))
        image = image.resize((render_w, render_h), resample)
    ...
    def _readme_crop_bounds(
        ...
        *,
        ...
        if rr.size:
            pad = max(12, int(round(min(shape) * 0.025)))
    ...
    def _readme_nav_mask_image(
        ...
        *,
        ...
        raw_nav = self._room_debug_array("voxel_nav_final_free_xy", shape, bool)
        if not np.any(raw_nav):
            raw_nav = self._room_debug_array("voxel_nav_free_raw_xy", shape, bool)
        clearance_removed = raw_nav & ~nav & obs & ~occ & ~outside
        if np.any(clearance_removed):
            removed_color = np.asarray((255, 128, 40), dtype=np.float32)
            image[clearance_removed] = np.clip(
                image[clearance_removed].astype(np.float32) * 0.25 + removed_color[None, :] * 0.75,
                0,
                255,
            ).astype(np.uint8)
        low_clearance = self._room_debug_array("astar_clearance_low_cells", shape, bool)
        low_clearance_mask = low_clearance & nav & obs & ~outside
        if np.any(low_clearance_mask):
            clearance_color = np.asarray((255, 190, 40), dtype=np.float32)
            image[low_clearance_mask] = np.clip(
                image[low_clearance_mask].astype(np.float32) * 0.35 + clearance_color[None, :] * 0.65,
                0,
                255,
            ).astype(np.uint8)
        image[occ] = (88, 88, 88)
    ...
    def _readme_vertical_free_image(self, shape: Tuple[int, int], crop_bounds: Tuple[int, int, int, int]) -> Image.Image:
        ...
        raw_door_seed = self._room_debug_array("voxel_door_raw_seed_mask", shape, bool)
        kept_door_seed = self._room_debug_array("voxel_door_seed_model_keep_mask", shape, bool)
        rejected_door_seed = self._room_debug_array("voxel_door_seed_model_reject_mask", shape, bool)
        model_filter_active = bool(np.any(kept_door_seed) or np.any(rejected_door_seed))
        door_seed = (
            (kept_door_seed if model_filter_active else raw_door_seed)
        ...
        image[rejected_door_seed] = MODEL_REJECTED_DOOR_SEED_COLOR
        r0, r1, c0, c1 = crop_bounds
        return Image.fromarray(image[r0:r1, c0:c1])
    ...
    def _render_voxel_roomseg_panel(
        ...
        *,
        ...
        raw_door_seed = self._room_debug_array("voxel_door_raw_seed_mask", shape, bool)
        if not np.any(raw_door_seed):
            raw_door_seed = self._room_debug_array("voxel_raw_door_seed_mask", shape, bool)
        model_keep_door_seed = self._room_debug_array("voxel_door_seed_model_keep_mask", shape, bool)
        model_rejected_door_seed = self._room_debug_array("voxel_door_seed_model_reject_mask", shape, bool)
        if np.any(model_keep_door_seed) or np.any(model_rejected_door_seed):
            door_seed = model_keep_door_seed
        ...
        canvas[model_rejected_door_seed] = MODEL_REJECTED_DOOR_SEED_COLOR
        ...
        if missing:
            ...
            raw_door_seed_count = int(np.count_nonzero(raw_door_seed))
            door_seed_count = int(np.count_nonzero(door_seed))
            model_rejected_door_seed_count = int(np.count_nonzero(model_rejected_door_seed))
            ...
            title_a = "voxel v32 | raw=%d keep=%d reject=%d cc=%d clusters=%d prim=%d ext=%d cut=%d" % (
                raw_door_seed_count,
                door_seed_count,
                model_rejected_door_seed_count,
        ...
        layers = [
            self._overlay_record(
                ...
                raw_door_seed_cells=int(np.count_nonzero(raw_door_seed)),
                door_seed_cells=int(np.count_nonzero(door_seed)),
                model_kept_door_seed_cells=int(np.count_nonzero(model_keep_door_seed)),
                model_rejected_door_seed_cells=int(np.count_nonzero(model_rejected_door_seed)),
            ...
            self._overlay_record("voxel_door_seed_model_rejected", bool(np.any(model_rejected_door_seed)), MODEL_REJECTED_DOOR_SEED_COLOR, int(np.count_nonzero(model_rejected_door_seed)), "raw door seed cells rejected by the learned classifier; dark crimson is distinct from bright-red strict wall"),
    ...
    def _voxel_roomseg_legend(self, draw: ImageDraw.ImageDraw, xy: Tuple[int, int]) -> None:
        ...
        items = [
            ...
            ((0, 80, 255), "door seed"),
            (MODEL_REJECTED_DOOR_SEED_COLOR, "model rejected seed"),
