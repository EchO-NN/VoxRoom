...
REJECTED_BY_LEARNED_FILTER = "rejected_by_learned_filter"
REJECTED_OUTSIDE_ROOMSEG_DOMAIN = "rejected_outside_roomseg_domain"
...
class VoxelDoorDetectorConfig:
    ...
    lower_free_use_in_range_unknown: bool = True
    ...
    min_upper_occupied_cells: int = 8
    door_seed_sensor_range_count_threshold: int = 0
    upper_solid_use_in_range_unknown: bool = True
...
def _door_scan_z_bounds(voxel_grid: VoxelOccupancyGrid3D, cfg: VoxelDoorDetectorConfig) -> tuple[float, float, str]:
    ...
    top = float(voxel_grid.z_max_m)
    if not np.isfinite(z_min) or not np.isfinite(top) or top < z_min:
        raise ValueError("door seed storage z range must be finite and satisfy z_max >= z_min")
    return float(z_min), float(top), "storage_z_max"
...
def classify_voxel_door_seeds(
    *,
    ...
    debug = {
        ...
        "voxel_door_seed_ceiling_filter_version": "highest_occupied_column_top_v1",
        "voxel_door_seed_method": str(cfg.seed_method),
        "voxel_door_seed_column_top_mode": "highest_occupied_voxel",
        "voxel_door_seed_navigation_free_gate_removed": True,
...
def rebuild_voxel_door_seed_result(
    raw_result: VoxelDoorSeedResult,
    filtered_seed_mask: np.ndarray,
    *,
    seed_connectivity: int,
    model_probability_xy: np.ndarray | None = None,
    eligible_raw_seed_mask: np.ndarray | None = None,
) -> VoxelDoorSeedResult:
    """Rebuild every seed-dependent field after a learned mask filter."""

    raw_mask = np.asarray(raw_result.door_seed_mask, dtype=bool)
    eligible_raw = raw_mask if eligible_raw_seed_mask is None else np.asarray(eligible_raw_seed_mask, dtype=bool)
    filtered = np.asarray(filtered_seed_mask, dtype=bool)
    if filtered.shape != raw_mask.shape:
        raise ValueError("filtered_seed_mask must match raw door seed shape")
    if eligible_raw.shape != raw_mask.shape or np.any(eligible_raw & ~raw_mask):
        raise ValueError("eligible_raw_seed_mask must be a subset of raw door seeds")
    if np.any(filtered & ~eligible_raw):
        raise ValueError("learned filtering may remove raw seeds but may not create new seeds")
    labels, component_count = ndimage.label(filtered, structure=conn(int(seed_connectivity)))
    reject_reason = np.asarray(raw_result.door_seed_reject_reason_map, dtype=np.uint8).copy()
    domain_reject = raw_mask & ~eligible_raw
    model_reject = eligible_raw & ~filtered
    reject_reason[domain_reject] = _seed_reject_code(REJECTED_OUTSIDE_ROOMSEG_DOMAIN)
    reject_reason[model_reject] = _seed_reject_code(REJECTED_BY_LEARNED_FILTER)
    evidence = [
        item
        for item in raw_result.seed_evidence
        if 0 <= int(item.row) < filtered.shape[0]
        and 0 <= int(item.col) < filtered.shape[1]
        and bool(filtered[int(item.row), int(item.col)])
    ]
    rejected_counts = Counter(
        _seed_reject_reason_from_code(int(code))
        for code in reject_reason.ravel()
        if int(code) not in {0, 1}
    )
    debug = dict(raw_result.debug)
    debug.update(
        {
            "voxel_door_raw_seed_mask": eligible_raw.copy(),
            "voxel_door_seed_mask": filtered.copy(),
            "voxel_door_seed_component_map": labels.astype(np.int32),
            "voxel_door_seed_reject_reason_map": reject_reason.copy(),
            "voxel_door_seed_cells_before_model": int(np.count_nonzero(eligible_raw)),
            "voxel_door_seed_outside_domain_rejected_cells": int(np.count_nonzero(domain_reject)),
            "voxel_door_seed_cells": int(np.count_nonzero(filtered)),
            "voxel_door_seed_component_count_after_model": int(component_count),
            "voxel_door_seed_model_keep_mask": filtered.copy(),
            "voxel_door_seed_model_reject_mask": model_reject.copy(),
            "voxel_door_seed_model_rejected_cells": int(np.count_nonzero(model_reject)),
            "voxel_door_seed_evidence": [item.to_dict() for item in evidence[:2048]],
            "voxel_door_seed_reject_reason_counts": dict(rejected_counts),
        }
    )
    if model_probability_xy is not None:
        probability = np.asarray(model_probability_xy, dtype=np.float32)
        if probability.shape != raw_mask.shape:
            raise ValueError("model_probability_xy must match raw door seed shape")
        debug["voxel_door_seed_model_probability_xy"] = probability.copy()
    debug.update(_seed_reject_reason_maps(reject_reason))
    return VoxelDoorSeedResult(
        door_seed_mask=filtered.copy(),
        door_seed_component_map=labels.astype(np.int32),
        door_seed_reject_reason_map=reject_reason,
        lower_free_cells_xy=np.asarray(raw_result.lower_free_cells_xy).copy(),
        top_occupied_cells_xy=np.asarray(raw_result.top_occupied_cells_xy).copy(),
        first_occupied_z_xy=np.asarray(raw_result.first_occupied_z_xy).copy(),
        unknown_tail_cells_xy=np.asarray(raw_result.unknown_tail_cells_xy).copy(),
...
def classify_voxel_door_seeds_centroid_ratio_vectorized(
    ...
    *,
    ...
    is_effective_observed = is_free | is_occ | is_unknown

    lower_unknown_evidence = (
        is_unknown
        if bool(getattr(cfg, "lower_free_use_in_range_unknown", False))
        else np.zeros_like(is_free, dtype=bool)
    )
    lower_free_evidence = is_free | lower_unknown_evidence
    ...
    has_occupied = np.any(is_occ, axis=0)
    highest_occupied_idx = np.full(column_count, -1, dtype=np.int32)
    if np.any(has_occupied):
        highest_occupied_idx[has_occupied] = (
            z_count - 1 - np.argmax(is_occ[::-1, has_occupied], axis=0)
        ).astype(np.int32)
    highest_occupied_z = np.full(column_count, np.nan, dtype=np.float32)
    highest_occupied_z[has_occupied] = z[highest_occupied_idx[has_occupied]]
    has_upper_occ = has_occupied & (highest_occupied_idx >= turn_pos)
    upper_position = (
        (z_rows >= np.clip(turn_pos, 0, z_count)[None, :])
        & (z_rows <= highest_occupied_idx[None, :])
        & has_upper_occ[None, :]
    )

    use_unknown_as_upper_solid = bool(getattr(cfg, "upper_solid_use_in_range_unknown", False)) or bool(
        getattr(cfg, "upper_solid_use_out_of_range_unknown", False)
    )
    upper_unknown_solid = is_unknown if use_unknown_as_upper_solid else np.zeros_like(is_occ, dtype=bool)
    is_upper_solid = is_occ | upper_unknown_solid

    free_cum = np.cumsum(lower_free_evidence, axis=0, dtype=np.int32)
    ...
    upper_occ = np.sum(is_occ & upper_position, axis=0).astype(np.int32)
    upper_solid = np.sum(is_upper_solid & upper_position, axis=0).astype(np.int32)
    upper_obs = np.sum(is_effective_observed & upper_position, axis=0).astype(np.int32)
    upper_total_active = np.sum(upper_position, axis=0).astype(np.int32)
    upper_in_range_unknown = np.sum(is_in_range_unknown & upper_position, axis=0).astype(np.int32)
    upper_out_of_range_unknown = np.sum(is_out_of_range_unknown & upper_position, axis=0).astype(np.int32)
    upper_unknown = (upper_in_range_unknown + upper_out_of_range_unknown).astype(np.int32)
    upper_free = np.sum(is_free & upper_position, axis=0).astype(np.int32)
    ...
    upper_unknown_ratio_active = np.zeros(column_count, dtype=np.float32)
    ...
    np.divide(upper_unknown.astype(np.float32), np.maximum(upper_total_active, 1), out=upper_unknown_ratio_active, where=upper_total_active > 0)
    ...
    min_upper_actual_occ = int(cfg.min_upper_occupied_cells)
    ...
    reject(upper_unknown_ratio_active > float(getattr(cfg, "upper_in_range_unknown_ratio_max", 0.25)), "upper_in_range_unknown_ratio_too_high")
    ...
    first_occ_z = highest_occupied_z
    ...
    debug_maps = {
        ...
        "voxel_door_upper_unknown_ratio_active_xy": upper_unknown_ratio_active.reshape(shape).astype(np.float32),
        "voxel_door_upper_free_ratio_active_xy": upper_free_ratio_active.reshape(shape).astype(np.float32),
        "voxel_door_highest_occupied_z_xy": highest_occupied_z.reshape(shape).astype(np.float32),
        "voxel_door_first_upper_occupied_z_xy": highest_occupied_z.reshape(shape).astype(np.float32),
        "voxel_door_upper_non_ceiling_actual_occupied_count_xy": np.clip(upper_occ, 0, np.iinfo(np.uint16).max).reshape(shape).astype(np.uint16),
        ...
        "voxel_door_upper_solid_uses_in_range_unknown": np.full(shape, bool(use_unknown_as_upper_solid), dtype=bool),
        "voxel_door_upper_solid_uses_out_of_range_unknown": np.full(shape, bool(use_unknown_as_upper_solid), dtype=bool),
    }
...
_SEED_REJECT_CODES = {
    ...
    REJECTED_BY_LEARNED_FILTER: 30,
    REJECTED_OUTSIDE_ROOMSEG_DOMAIN: 31,
}
...
def _seed_reject_reason_maps(reason_map: np.ndarray) -> dict[str, np.ndarray]:
    return {
        ...
        "voxel_door_seed_rejected_by_learned_filter_cells": np.asarray(reason_map == _SEED_REJECT_CODES[REJECTED_BY_LEARNED_FILTER], dtype=bool),
        "voxel_door_seed_rejected_outside_roomseg_domain_cells": np.asarray(
            reason_map == _SEED_REJECT_CODES[REJECTED_OUTSIDE_ROOMSEG_DOMAIN], dtype=bool
        ),
    }
...
def _empty_seed_debug_maps(shape: tuple[int, int]) -> dict[str, np.ndarray]:
    return {
        ...
        "voxel_door_upper_unknown_ratio_active_xy": np.zeros(shape, dtype=np.float32),
        "voxel_door_upper_free_ratio_active_xy": np.zeros(shape, dtype=np.float32),
        "voxel_door_highest_occupied_z_xy": np.full(shape, np.nan, dtype=np.float32),
