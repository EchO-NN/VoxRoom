    ...
    *,
    ...
    if bool(getattr(cfg, "count_in_range_unknown_as_occupied_for_wall", True)):
        generalized_occupied_count = np.minimum(
            occupied_count.astype(np.uint32) + unknown_count.astype(np.uint32),
            np.iinfo(np.uint16).max,
        ).astype(np.uint16)
    else:
    ...
    outside_unknown_dominant = (
        unknown_ratio >= float(getattr(cfg, "unknown_ratio_min_for_xy_unknown", cfg.unknown_ratio_min_for_xy_unknown))
    ) | (
    ...
    wall_line_support_unknown_hard_block = unknown_ratio >= float(cfg.wall_line_support_unknown_ratio_hard_max)
    ...
    some_unknown = unknown_ratio >= float(getattr(cfg, "frontier_residual_unknown_ratio_min", 0.25))
    ...
    return {
        ...
        "wall_from_in_range_unknown": (
            wall
            & (unknown_count > 0)
            & bool(getattr(cfg, "count_in_range_unknown_as_occupied_for_wall", True))
        ).astype(bool),
