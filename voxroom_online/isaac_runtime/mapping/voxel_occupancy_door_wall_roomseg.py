...
VOXEL_OCCUPANCY_ROOMSEG_CONTEXT = "voxel_occupancy_door_wall_v33"
...
class VoxelOccupancyDoorWallRoomSegConfig:
    ...
    door_seed_learning: DoorSeedLearningConfig = field(default_factory=DoorSeedLearningConfig)
    ...
    def from_mapping(cls, data: Mapping[str, object] | None = None, **overrides: object) -> "VoxelOccupancyDoorWallRoomSegConfig":
        ...
        nested = {
            ...
            "door_seed_learning": DoorSeedLearningConfig.from_mapping(raw_root.get("door_seed_learning", {})),
...
def _expand_raw_seed_result_to_stage_union(
    stage: DoorSeedStageResult,
    *,
    seed_connectivity: int,
) -> DoorSeedStageResult:
    """Make the TVARS-added cells first-class raw seeds before model filtering."""

    union_mask = np.asarray(stage.raw_seed_mask_xy, dtype=bool)
    ...
    reject_reason = np.asarray(
        raw_result.door_seed_reject_reason_map,
        dtype=np.uint8,
    ).copy()
    reject_reason[union_mask] = 1
    expanded_result = replace(
        raw_result,
        door_seed_mask=union_mask.copy(),
        door_seed_component_map=labels.astype(np.int32),
        door_seed_reject_reason_map=reject_reason,
        debug={
            **dict(raw_result.debug),
            **dict(stage.debug),
            "voxel_door_raw_seed_mask": union_mask.copy(),
            "voxel_door_seed_mask": union_mask.copy(),
            "voxel_door_seed_component_map": labels.astype(np.int32),
            "voxel_door_seed_reject_reason_map": reject_reason.copy(),
            "voxel_door_seed_cells_before_model": int(np.count_nonzero(union_mask)),
            "voxel_door_seed_cells": int(np.count_nonzero(union_mask)),
            "voxel_door_seed_component_count_before_model": int(component_count),
            "voxel_door_seed_union_added_tvars_cells": int(
                np.count_nonzero(union_mask & ~original_mask)
            ),
        },
    )
    return replace(stage, raw_seed_result=expanded_result)
...
class VoxelOccupancyDoorWallRoomSegmenter:
    ...
    def __init__(self, config: VoxelOccupancyDoorWallRoomSegConfig | Mapping[str, object] | None = None, map_info: MapInfo | None = None):
        ...
        self.door_seed_inference_engine = (
            DoorSeedInferenceEngine(self.config.door_seed_learning)
...
def run_voxel_occupancy_door_wall_roomseg(
    *,
    ...
    agent_rc: Sequence[int] | None = None,
    agent_yaw_deg: float | None = None,
    ...
    door_seed_stage = extract_door_seed_stage(
        voxel_grid=voxel_grid,
        navigation_free_mask=nav_free_mask,
        navigation_obstacle_mask=nav_obstacle_mask,
        unknown_mask=nav_unknown_mask,
        door_seed_no_clearance_free_mask=door_seed_free_mask,
        resolution_m=float(resolution_m),
        voxel_evidence_config=cfg.voxel_evidence,
        door_config=cfg.door,
    )
    if door_seed_raw_seed_accumulator is not None:
        if agent_rc is None or len(agent_rc) < 2:
            raise ValueError(
                "VoxRoom+TVARS raw-seed inference requires the current agent grid"
            )
        if agent_yaw_deg is None:
            raise ValueError(
                "VoxRoom+TVARS raw-seed inference requires the current agent yaw"
            )
        door_seed_stage = door_seed_raw_seed_accumulator.combine(
            door_seed_stage,
            agent_rc=(int(agent_rc[0]), int(agent_rc[1])),
            yaw_deg=float(agent_yaw_deg),
            resolution_m=float(resolution_m),
            use_history=bool(cfg.door_seed_learning.save_full_voxel_milestones),
        )
        door_seed_stage = _expand_raw_seed_result_to_stage_union(
            door_seed_stage,
            seed_connectivity=int(cfg.door.seed_connectivity),
        )
    evidence = door_seed_stage.evidence
    outside_roomseg_debug = {
        key: value
        for key, value in door_seed_stage.debug.items()
        if str(key).startswith("voxel_outside_")
    }
    raw_door_seed_mask = np.asarray(door_seed_stage.raw_seed_mask_xy, dtype=bool)
    door_seed_result = door_seed_stage.raw_seed_result
    if door_seed_inference_engine is not None:
        filter_output = door_seed_inference_engine.filter_stage(
            stage=door_seed_stage,
            voxel_grid=voxel_grid,
            seed_connectivity=int(cfg.door.seed_connectivity),
        )
        door_seed_result = filter_output.seed_result
    ...
    vertical_partition_labels, small_room_label_debug = _filter_small_room_labels(
        labels,
        min_area_m2=float(cfg.min_room_area_m2),
        resolution_m=float(resolution_m),
    )
    labels, navigation_projection_debug = _project_room_labels_to_navigation_free(
        vertical_partition_labels,
        navigation_free_mask=nav_free_mask,
    )
    ...
    layers = {
        ...
        "voxel_door_seed_model_keep_mask": np.asarray(
            door_seed_result.debug.get("voxel_door_seed_model_keep_mask", door_seed_mask),
            dtype=bool,
        ) & ~outside_xy,
        "voxel_door_seed_model_reject_mask": np.asarray(
            door_seed_result.debug.get("voxel_door_seed_model_reject_mask", np.zeros(shape, dtype=bool)),
            dtype=bool,
        ) & ~outside_xy,
        ...
        "voxel_vertical_free_partition_room_label_map": vertical_partition_labels,
        "voxel_navigation_projected_room_label_map": labels,
...
def _filter_small_room_labels(
    ...
    *,
    ...
    min_cells = (
        int(np.ceil((min_area / cell_area) - 1.0e-9))
        if min_area > 0.0
        else 0
    )
    debug: dict[str, object] = {
...
def _project_room_labels_to_navigation_free(
    labels: np.ndarray,
    *,
    navigation_free_mask: np.ndarray,
) -> tuple[np.ndarray, dict[str, object]]:
    """Project Vertical Free partition labels onto the navigation-free map."""

    source = np.asarray(labels, dtype=np.int32)
    navigation_free = np.asarray(navigation_free_mask, dtype=bool)
    if source.ndim != 2 or navigation_free.shape != source.shape:
        raise ValueError(
            "room labels and navigation-free projection must be matching 2D arrays"
        )
    projected = source.copy()
    projected[~navigation_free] = 0
    projected = relabel_compact(projected.astype(np.int32))
    return projected, {
        "partition_source": "voxel_vertical_free_xy",
        "projection_target": "voxel_nav_free_xy",
        "vertical_partition_room_count": int(_room_count(source)),
        "vertical_partition_labeled_cells": int(np.count_nonzero(source)),
        "navigation_projected_room_count": int(_room_count(projected)),
        "navigation_projected_labeled_cells": int(np.count_nonzero(projected)),
        "labels_removed_by_navigation_projection_cells": int(
            np.count_nonzero((source > 0) & ~navigation_free)
        ),
    }
