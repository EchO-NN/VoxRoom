...
class OnlineMapper:
    def __init__(
        ...
        allow_disabled_voxel_grid_for_sensor_smoke_test: bool = False,
    ):
        ...
        voxel_cfg_raw["depth_min_m"] = self.depth_min_m
        voxel_cfg_raw["depth_max_m"] = self.depth_max_m
        ...
        # Preserve the original nvblox-only contract by default.  The explicit
        # opt-in is used only by the ZED sensor smoke test so depth semantics,
        # extrinsics, and visual-inertial tracking can be checked before CUDA and
        # nvblox are introduced.  Keeping a tiny disabled grid preserves the
        # debug/state interfaces without running voxel integration.
        allow_disabled_voxel_grid = bool(allow_disabled_voxel_grid_for_sensor_smoke_test)
        if not bool(self.voxel_grid_config.enabled) and not allow_disabled_voxel_grid:
            raise RuntimeError("nvblox-only VoxRoom requires mapping.room_segmentation.voxel_grid.enabled=true")
        if bool(self.voxel_grid_config.enabled) and not bool(self.voxel_grid_drives_navigation):
            raise RuntimeError("nvblox-only VoxRoom requires voxel_grid_drives_navigation=true")
        if not bool(self.voxel_grid_config.enabled):
            self.voxel_grid_drives_navigation = False
    ...
    def update_simple_radius(self, base_pose_world: Tuple[float, float, float, float], radius_m: float = 1.5) -> OnlineGridMap:
        # Lightweight 2D smoke-test helper: mark a local disk free.
    ...
    def _mark_sensor_effective_range(
        ...
        *,
        ...
        if sparse_enabled and pixels.size:
            camera_transform_world = pose_world_to_matrix(camera_pose_world)
            ...
            points_world = transform_points(points_cam, camera_transform_world)
            sparse_updates = self.voxel_grid.mark_sensor_effective_range_rays(
                camera_origin_world=np.asarray(camera_transform_world[:3, 3], dtype=np.float32),
    ...
    def update(
        ...
        camera_transform_world = pose_world_to_matrix(camera_pose_world)
        camera_origin_world = camera_transform_world[:3, 3]
        ...
        sensor_effective_range_debug = self._mark_sensor_effective_range(
            pixels_uv=pixels,
            intr=intr,
            camera_pose_world=camera_transform_world,
            floor_z=floor_z,
        )
        timings["sensor_effective_range_ms"] = float(sensor_effective_range_debug.get("sensor_effective_range_ms", 0.0) or 0.0)
        no_integrable_depth_rays = int(np.count_nonzero(ray_mask)) == 0
        if no_integrable_depth_rays:
            ray_pixels = np.zeros((0, 2), dtype=np.float32)
            ray_depth = np.zeros((0,), dtype=np.float32)
            ray_endpoint_is_hit = np.zeros((0,), dtype=bool)
            hit_pixels = np.zeros((0, 2), dtype=np.float32)
            hit_depth = np.zeros((0,), dtype=np.float32)
        else:
        ...
        if hit_pixels.size:
            ...
            hit_points_world = transform_points(hit_points_cam, camera_transform_world)
        else:
        ...
        origin_cell = self.grid.world_to_grid(float(camera_origin_world[0]), float(camera_origin_world[1]))
        ...
        if not is_inside_grid(origin_cell[0], origin_cell[1], self.grid.map_info):
            origin_cell = (0, 0)
            hit_in_bounds = np.zeros_like(hit_in_bounds, dtype=bool)
        ...
        voxel_endpoint_dirty_flat = np.zeros((0,), dtype=np.int64)
        if bool(self.voxel_grid_config.enabled):
            voxel_endpoint_dirty_flat = self._mark_voxel_navigation_endpoint_evidence(rows_cols, rel_z)
        ...
        if bool(self.voxel_grid_config.enabled):
            status = "locked" if bool(self.last_ceiling_height_estimate.locked) else ("stable" if bool(self.last_ceiling_height_estimate.stable) else "unavailable")
            self.voxel_grid.set_active_z_from_ceiling(self.last_ceiling_height_estimate.height_m, status=status)
            backend = self.voxel_grid._resolve_integration_backend()
            if backend != "nvblox_fast_dda":
                raise RuntimeError(f"Unsupported integration backend in nvblox-only branch: {backend!r}")
            nvblox_depth_arr = depth_arr
            voxel_stats = self.voxel_grid.integrate_depth_image(
                depth=nvblox_depth_arr,
                intr=intr,
                camera_pose_world=camera_transform_world,
                floor_z=floor_z,
            )
        ...
        camera_rel_z = float(camera_origin_world[2]) - floor_z
        ...
        voxel_endpoint_decay_dirty_flat = np.zeros((0,), dtype=np.int64)
        if bool(self.voxel_grid_config.enabled):
            voxel_endpoint_decay_dirty_flat = self._decay_voxel_navigation_endpoint_evidence(free_unique)
        ...
        if bool(self.voxel_grid_config.enabled):
            ...
            endpoint_dirty_flat = (
                np.unique(np.concatenate([voxel_endpoint_dirty_flat, voxel_endpoint_decay_dirty_flat])).astype(np.int64, copy=False)
                if (voxel_endpoint_dirty_flat.size or voxel_endpoint_decay_dirty_flat.size)
                else np.zeros((0,), dtype=np.int64)
            )
            if endpoint_dirty_flat.size:
                hw = int(self.grid.map_info.height) * int(self.grid.map_info.width)
                endpoint_dirty_flat = endpoint_dirty_flat[(endpoint_dirty_flat >= 0) & (endpoint_dirty_flat < hw)]
                if endpoint_dirty_flat.size:
                    raw_dirty = getattr(self.voxel_grid, "last_dirty_rc_flags", None)
                    if raw_dirty is None or int(np.asarray(raw_dirty).reshape(-1).size) != hw:
                        dirty_flags = np.zeros(hw, dtype=np.uint8)
                    else:
                        dirty_flags = np.asarray(raw_dirty, dtype=np.uint8).reshape(-1).copy()
                    dirty_flags[endpoint_dirty_flat] = 1
                    self.voxel_grid.last_dirty_rc_flags = dirty_flags
            self.voxel_nav_endpoint_decay_debug["voxel_nav_endpoint_dirty_cells"] = int(endpoint_dirty_flat.size)
            ...
            voxel_nav_projection.debug.update(dict(self.voxel_nav_endpoint_decay_debug))
        ...
        self.last_debug_stats.update(
            {
                ...
                "nvblox_depth_frame_shape": list(nvblox_depth_arr.shape) if bool(self.voxel_grid_config.enabled) else [],
                "nvblox_depth_frame_finite_count": int(np.count_nonzero(np.isfinite(nvblox_depth_arr))) if bool(self.voxel_grid_config.enabled) else 0,
                "nvblox_depth_frame_nan_count": int(np.count_nonzero(np.isnan(nvblox_depth_arr))) if bool(self.voxel_grid_config.enabled) else 0,
        ...
        voxel_integration_debug = self.voxel_grid.last_integration_stats.to_dict()
        voxel_navigation_debug = dict(self.voxel_grid.last_navigation_debug)
        for key in (
            "voxel_integration_backend",
            "nvblox_query_mode",
            "nvblox_query_voxels",
            "nvblox_dirty_rc_count",
        ):
            if key in voxel_integration_debug:
                self.last_debug_stats[key] = voxel_integration_debug[key]
        for key in ("voxel_nav_free_cells", "voxel_nav_observed_cells"):
            if key in voxel_navigation_debug:
                self.last_debug_stats[key] = voxel_navigation_debug[key]
        self.last_debug_stats.update(
    ...
    def update_nearfield_topdown(
        ...
        pixels = np.stack([uu.reshape(-1), vv.reshape(-1)], axis=1).astype(np.float32)
        sampled_depth = depth_arr[pixels[:, 1].astype(np.int64), pixels[:, 0].astype(np.int64)]
        cam_z = float(pose_world_to_matrix(camera_pose_world)[2, 3])
        floor_z = float(base_pose_world[2])
        ...
        stats = {
            ...
            "camera_pose_world": _pose_debug_value(camera_pose_world),
            "camera_height_m": float(camera_height),
            "radius_m": float(radius),
            "ignore_radius_m": float(ignore_radius),
    ...
    def _mark_voxel_navigation_endpoint_evidence(self, rows_cols: np.ndarray, rel_z: np.ndarray) -> np.ndarray:
    ...
    def _decay_voxel_navigation_endpoint_evidence(self, free_flat: np.ndarray) -> np.ndarray:
        cfg = self.voxel_navigation_projection_config
        free_unique = _unique_flat_array(free_flat)
        decayed_cells = 0
        stale_cleared = 0
        dirty_flat = np.zeros((0,), dtype=np.int64)
        if free_unique.size and bool(cfg.get("occupied_use_endpoint_hysteresis", True)):
            ...
            if free_unique.size:
                ...
                if np.any(protected):
                    ...
                    dirty_flat = protected_flat.astype(np.int64, copy=False)
...
def _pose_debug_value(pose_world: Sequence[float] | np.ndarray) -> list:
    return np.asarray(pose_world, dtype=np.float64).tolist()
